...
import assigment1.tucn.cs.DAL.CoursBuilder;
...
public abstract class Repository implements IRepository {
			...
			try (ResultSet resultSet = statement.executeQuery();) {
				...
				while (resultSet.next()) {
					...
					case USER:
						...
						break;
					case COURS:
						objects.add(getCoursFromResultSet(resultSet));
						break;
					default:
						break;
					}
	...
	public Cours getCoursFromResultSet(ResultSet rs) throws SQLException {
		return new CoursBuilder().setId(rs.getLong("id")).setCoursName(rs.getString("course_name"))
				.setExamDate(rs.getDate("examDate")).setTeacherId(rs.getLong("Teacher_id")).build();
	}
}
